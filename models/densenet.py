...
def conv_block(input,growth_rate,dropout_rate=None,weight_decay=1e-4):
    ...
    x = Activation('relu')(x)
    x = Conv2D(growth_rate,(3,3),kernel_initializer='he_normal', padding = 'same',
...
def dense_cnn(nclass):
    ...
    _nb_filter = 64
    # conv 64  5*5 s=2
    input = Input(shape=(_img_h,None,1),name='the_input')
    ...
    # 64 +  8 * 8 = 128
    x ,_nb_filter = dense_block(x,8,_nb_filter,8, droput_rate=0., weight_decay=_weight_decay)
    #128
